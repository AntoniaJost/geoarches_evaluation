...
from plot.functional.spatial import lambert_conformal_projection_plot, azimuthal_equidistant_projection_plot
...
def compute_anomaly(
    ...
    if baseline_period is None:
        # Basically full climatology, so just take mean over all time steps
        baseline_data = data.sel(time=data.time.values, method="nearest")
    else:
...
class XYMaps:
    ...
    def eval_against_reference(self, data, reference_data):
        # Weight data and reference by latitude weights
        ...
        # copy data arrays to avoid modifying original data
        weighted_data = data.copy()
        weighted_reference = reference_data.copy()
        weighted_data.values = data.values * w
        weighted_reference.values = reference_data.values * w
        weighted_data = weighted_data.values
        weighted_reference = weighted_reference.values
        weighted_data = weighted_data[np.isnan(weighted_reference) == False]
        weighted_reference = weighted_reference[np.isnan(weighted_reference) == False]
        # Compute weighted rmse
        rmse = np.sqrt(np.mean((weighted_data - weighted_reference) ** 2))
    
        # Bias 
        bias = (weighted_data - weighted_reference).mean()
        return rmse, bias
    ...
    def evaluate(self, data_containers):
        ...
        self.reference_label = data_containers[idx_ref[0]].model_label if idx_ref else None
        ...
        for ts in self.temporal_selection:
            ...
            for variable in self.variables:
                ...
                for data_container in data_containers:
                    ...
                    maps[data_container.model_label] = (data, output_path, var_name)

                for model_label, (data, output_path, var_name) in maps.items():
                    # Eval against reference if available
                    ...
                    self.plotter.plot(
                        ...
                        info={
                            "rmse": f"{rmse:.2f}" if self.reference_data is not None else "N/A",
                            "bias": f"{bias:.2f}" if self.reference_data is not None else "N/A"
                        }
                    )

class LatTimeMap(XYMaps):
    def __init__(self, variables, xdim="time", ydim="lat", temporal_selection: list = ['annual'], plotter_kwargs: dict = {}, frequency: str = "monthly"):
        super().__init__(variables, xdim, ydim, temporal_selection, plotter_kwargs, frequency)
    ...
    def compute(self, data_container, temporal_dim, var: dict, frequency: str = "monthly"):
        ...
        data = data.transpose("lat", "time")
        return data.time.values, data.lat.values, data.values

    def evaluate(self, data_containers):
    
        print(f"Computing {self.xdim}-{self.ydim} map")
        print("-" * 72)
        # Do computation here
        for variable in self.variables:
            name, pressure_level = variable['name'], variable['pressure_level']
            ...
            for data_container in data_containers:
                ...
                var_name = f"{name}_{pressure_level}Pa" if pressure_level is not None else name
                # Add temporal selection to output path for plotting
                output_path = self.plotter.output_path
                maps[data_container.model_label] = (data, output_path, var_name)
            ...
            for model_label, (data, output_path, var_name) in maps.items():
                ...
                plt.imshow(data[2], cmap="coolwarm")
                plt.colorbar(label=f"{self.plotter.cmor_units.get(name, '')}", shrink=0.6, extend='both')
                # convert time labels to year only
                time_labels = [pd.to_datetime(t).year for t in data[0]]
                time_labels_clean = list(set(time_labels))
                time_labels_clean.sort()
                ticks = [time_labels.index(t) for t in time_labels_clean]
                ticks = ticks[::4]
                time_labels_clean = [str(t) for t in time_labels_clean]
                time_labels_clean = time_labels_clean[::4]
                plt.xticks(ticks=ticks, labels=time_labels_clean, rotation=45)
                plt.yticks(ticks=np.linspace(0, len(data[1]), 7), labels=np.linspace(90, -90, 7), rotation=45)
                plt.savefig(os.path.join(output_path, f"{model_label}_{var_name}.png"), bbox_inches='tight')
                plt.close(fig)
...
class Timeseries:
    ...
    def compute_latitude_weights(self, latitude, longitude): 
...
class SeasonalCycles(Timeseries):
    ...
    def compute(self, data: xr.DataArray):
        ...
        #w = self.compute_latitude_weights(latitude, longitude)

        # Weight and spatial average data
        #data.values = data.values * w
        data = data.mean(dim=["lat", "lon"])
        print("Data weighted and spatially averaged.", end="; ")
    ...
    def evaluate(self, data_containers):
        ...
        for variable in self.variables:
                name, pressure_level = variable['name'], variable['pressure_level']
                if pressure_level is not None:
                    print(f"--> Processing variable: {name} at pressure level "
                          f"{pressure_level} Pa")
                else:
                    print(f"--> Processing variable: {name}")
                ...
                standard_deviations = {}
                for data_container in data_containers:
                    print(f"... for model: {data_container.model_label}: ", end="")
                    data = data_container.get_variable_data(
                        name=name, 
                    ...
                    if "stat" in data.dims:
                        mean = data.sel(stat="mean", drop=True)
                        cycle = self.compute(mean)
                        std = data.sel(stat="std", drop=True)
                        std_cycle = self.compute(std)
                        standard_deviations[data_container.model_label] = std_cycle.values
                    else:
                        cycle = self.compute(data)

                    cycles[data_container.model_label] = \
                        (cycle.time.values, cycle.values)
...
class MonsoonIndices:
    def __init__(
            self, method="webster_yang", frequency="monthly", target_year=2024, baseline_period: tuple = None, 
            plotter_kwargs: dict = None):
        ...
        self.plotter_kwargs["output_path"] = os.path.join(
    ...
    def webster_yang(self, data_container, baseline=False):
        # Compute Webster-Yang monsoon index
        
        u850 = data_container.get_variable_data(
            name="ua", pressure_level=85000, frequency=self.frequency)
        u250 = data_container.get_variable_data(
            name="ua", pressure_level=25000, frequency=self.frequency)
        ...
        if baseline is False:
            ...
            print(f"Selecting baseline period: {self.baseline_period[0]} to {self.baseline_period[1]}")
            u850 = u850.sel(time=(u850.time.dt.year >= int(self.baseline_period[0])) &
                                    (u850.time.dt.year <= int(self.baseline_period[1])))
            u250 = u250.sel(time=(u250.time.dt.year >= int(self.baseline_period[0])) &
                                    (u250.time.dt.year <= int(self.baseline_period[1])))
        
    
        shear = u850 - u250
        index = shear.mean(["lat", "lon"])

        if self.frequency == "monthly":
            
            index = index.groupby("time.month").mean("time")
            index = index.rename({"month": "time"})

        else:
            index = index.groupby("time.dayofyear").mean("time")
            index = index.rename({"dayofyear": "time"})
        
        return index

    def compute(self, data_container, baseline=False):
    ...
    def evaluate(self, data_containers):
        ...
        indices = {}
        reference_label = next((dc.model_label for dc in data_containers if dc.is_reference), None)
        for data_container in data_containers:
            index = self.compute(data_container)
            indices[data_container.model_label] = (index.time.values, index.values) 
            print(f"Computed index for {data_container.model_label}.")

            if data_container.model_label == reference_label:
                reference_index = self.compute(data_container, baseline=True)
                reference = (reference_index.time.values, reference_index.values)
                print(f"Computed reference index for {reference_label}.")
        ...
        linestyles = {
            data_container.model_label: "-" for data_container in data_containers
        }

        if reference_label: 
            if self.baseline_period:
                b_reference_label = reference_label \
                    + f" ({self.baseline_period[0]}-{self.baseline_period[1]})"
            else:
                b_reference_label = reference_label + ""

            indices[b_reference_label] = reference

        colors[b_reference_label] = colors[reference_label]
        linestyles[b_reference_label] = "--"

        if self.frequency == "monthly":
            xticks = (range(0, 12), ["Jan", "Feb", "Mar", "Apr", "May", "Jun", 
                    "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"])
        elif self.frequency == "daily":
            xtick_labels = ["Jan", "Feb", "Mar", "Apr", "May", "Jun",
                           "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
            # Place xticks at the middle of each month
            xticks = ([0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334], xtick_labels)
        else:
            xticks = None
...
def compute_eof(data: xr.DataArray, n_modes: int = 1,  ):
    ...
    # Ensure that the leading mode has a negative correlation with the original data
    ...
    return eof_modes, U, S, Vt
    
class EOF:
    def __init__(self, n_modes: int = 1, data=None):
        self.n_modes = n_modes

    def compute_eofs(self, data: xr.DataArray, lat_slicer=None, lon_slicer=None):
        assert isinstance(data, xr.DataArray), "Data must be an xarray DataArray."
        self._data = data

        # Weight the data
        weights = compute_latitude_weights(
            self._data.lat.values, self._data.lon.values
        )
        self._data.values = self._data.values * weights
        ...
        self._anomaly = self._anomaly.rename({"year": "time"}).reset_index("time", drop=True)

        self._anomaly = self._anomaly.sel(
            lat=lat_slicer,
            lon=lon_slicer
        )
        ...
        # Compute eof modes via svd
        eof_modes, A, Lh, E = compute_eof(
            self._anomaly, n_modes=self.n_modes
        )

        # Normalize modes by their standard deviation
        # This ensures that the modes have unit variance
        self.eof_modes = eof_modes  
        self.norm_coeff = Lh * Lh / (self.eof_modes.shape[0] - 1)

    def eigenvals_timeseries(self):
        return self.eof_modes / np.std(self.eof_modes, axis=0, keepdims=True) 

    def project_eofs(self, anomaly: xr.DataArray, eof_modes: xr.DataArray):
        projection = np.empty((len(anomaly.lat), len(anomaly.lon)))
        anomaly_vals = anomaly.values
        mean_anomaly = anomaly_vals.mean(axis=0)

        projection = np.empty((len(anomaly.lat), len(anomaly.lon)))

        # This is expensive on large grids, consider parallelization or vectorization
        for i in range(len(anomaly.lat)):    
            for j in range(len(anomaly.lon)):
                grid_point_data = anomaly_vals[:, i, j]

                _, _, _, p, _= stats.linregress(grid_point_data, eof_modes[:, 0])
                #projection[i, j] = grid_point_data * slope + intercept # 

                if p < 0.05:  # Only consider significant correlations
                    projection[i, j] = np.dot(grid_point_data, eof_modes[:, 0]) 
                else:
                    projection[i, j] = 0.0
...
class AnnularModes:
    def __init__(
        ...
        if self.latitude_bands is not None:
            self.latitude_slicer = slice(
                max(self.latitude_bands), 
                min(self.latitude_bands))  
        else:
            self.latitude_slicer = slice(None)

        self.longitude_bands = longitude_bands
        if self.longitude_bands is not None:
            self.longitude_slicer = slice(
                min(self.longitude_bands), 
                max(self.longitude_bands))
        else:
            self.longitude_slicer = slice(None)
    ...
    def compute(self, data):
        ...
        # Select time
        data = self.slice_time(
            data, time=self.time
        )

        # Store latitudes and longitudes for later use in plotting
        ...
        data = compute_anomaly(
            data=data, baseline_period=self.baseline_period, 
            mean_groups=["time.year"], baseline_mean_groups=None, standardize=True
        )
        data = data.rename({"year": "time"}).reset_index("time", drop=True)
        #data = data.sel(
        #    lat=self.latitude_slicer,
        #    lon=self.longitude_slicer
        #)

        self.latitudes = data.lat.values
        self.longitudes = data.lon.values
        time = data.time.values
        projection = self.eof_op.project_eofs(data, eigenvals)

        return time, projection, eigenvals[:, 0]


    def regress_against_reference(self, model_projection, reference_projection):
        """
        Regress the model projection of eofs agains the referene projection.
        Obtain the Pearson-r-value, the p-value and the std_err

        Args:
            model_projection (np.ndarray): Model projection of EOFs.
            reference_projection (np.ndarray): Reference projection of EOFs.

        Returns:
            tuple: Pearson-r-value, p-value, and standard error.
        """
        linregress = stats.linregress(
            model_projection.flatten(), 
            reference_projection.flatten()
        )

        return linregress.rvalue, linregress.pvalue, linregress.stderr
    
    def evaluate(self, data_containers):
        """
        Evaluates all model / ground truths in a loop. 
        If a reference is specified, all other results are compared 
        against the reference. 

        Args:
            data_containers (CMORDataContainer): 
                List of CMORDataContainer instances.
        """
        eof_modes = {}
        projections = {}
        for data_container in data_containers:
            time, projection_data, eigenvals = self.compute(data_container)
            eof_modes[data_container.model_label] = (time, eigenvals)
            projections[data_container.model_label] = projection_data
        
        colors = {
            data_container.model_label: 
            data_container.model_color for data_container in data_containers
        }

        # Compute r value of between all eofs to the eof of the data container 
        # where is_reference is True
        ref_name = [dc.model_label for dc in data_containers if dc.is_reference]
        ref_name = ref_name[0] if len(ref_name) > 0 else None
        reference_eof = eof_modes[ref_name][1] if ref_name else None
        reference_projection = projections[ref_name] if ref_name else None
        ...
        for label, projection in projections.items():
            if label != ref_name and reference_projection is not None:
                ...
                std_ratio = np.std(projection ) / np.std(reference_projection)
                pm, rm = np.mean(projection), np.mean(reference_projection)
                centred_rmse = np.sqrt(np.mean((projection - pm - reference_projection - rm) ** 2))
                analysis[label] = {
                    "r": r,
                    "p": p,
                    "std_err": std_err,
            ...
            projection = xr.DataArray(
                projection, 
                coords=[self.latitudes, self.longitudes], dims=["lat", "lon"])
...
class NorthernAnnularMode(AnnularModes):
    ...
    def spatial_plot(self, projection, model_label, info_vals: dict = None):
        ...
        # replace 0  in projection with NaN for better visualization
        ...
        '''lambert_conformal_projection_plot(
            ...
            info_vals=info_vals,
            cut_boundary=False,
            levels=None, #np.linspace(-np.max(np.abs(projection)), np.max(np.abs(projection)), 10),
            fpath=os.path.join(output_path, f"{self.method}_{model_label}.png")
        )'''
    ...
    def compute(self, data_container):
        if self.var == "psl":
            data = data_container.get_variable_data(
                name="psl", frequency=self.frequency, pressure_level=None
            )
        elif self.var == "zg":
            data = data_container.get_variable_data(
...
class NorthernAtlanticOscillationIndex(AnnularModes):
    ...
    def compute(self, data_container):
        ...
        # Compute eofs and projection for PSL 
        psl = data_container.get_variable_data(
            name="psl", frequency=self.frequency, pressure_level=None
        )
...
class RadialSpectrum:
    def __init__(self, variables, reference_years: list, plotter_kwargs: dict = None):
        super().__init__()
    ...
    def compute(self, data_containers, variable_name, lvl):
        radial_spectra = {}
        for data_container in data_containers:
            data = data_container.get_variable_data(
                name=variable_name, frequency="monthly", pressure_level=lvl
            )

            radial_spectrum = self.compute_radial_spectrum(data)
            radial_spectra[data_container.model_label] = radial_spectrum
    ...
    def evaluate(self, data_containers):
        for var in self.variables:
            ...
            short_var_name = (
                name if pressure_level is None else f"{name} ({pressure_level}hPa)"
            )
            print(
            ...
            for i, (lbl, spectra) in enumerate(radial_spectra.items()):
                for j, (year, spectrum) in enumerate(spectra.items()):
                    color = list(data_containers[i].data_color)
...
class Distribution:
    def __init__(self, variables, time: list = None, plotter_kwargs: dict = None):
        super().__init__()
        self.variables = variables  
        self.time = time
        self.plotter_kwargs = plotter_kwargs or {}
    ...
    def compute(self, data: xr.DataArray):
        # Computes the distribution of a variable, e.g. by computing a histogram or a kernel density estimate.
        pass

    def visualize(self, distributions):
        # Visualizes the distribution of a variable, e.g. by plotting a histogram or a kernel density estimate.
        pass

    def evaluate(self, data_containers):
        # Evaluates the distribution of a variable for each data container and visualizes it.
        for var in self.variables:
            name, pressure_level = var
            ...
            for data_container in data_containers:
                ...
                if self.time is not None:
                    ...
                    var = var.sel(
                        time=slice(np.datetime64(start, 'ns'), np.datetime64(end, 'ns'))
                        )

                distribution = self.compute(var)
                distributions[data_container.model_label] = (
                    distribution, data_container.model_color
                )
            
            name = name if pressure_level is None else f"{name} ({pressure_level}hPa)"
            self.visualize(
                distributions, variable_name=name
            )
...
class Histogram(Distribution):
    def __init__(self, variables, time=None, plotter_kwargs = None):
        super().__init__(variables, time=time, plotter_kwargs=plotter_kwargs)
        self.figsize = plotter_kwargs.get("figsize", (10, 10))
        self.dpi = plotter_kwargs.get("dpi", 150)
        self.output_path = plotter_kwargs.get("output_path", ".")
        os.makedirs(self.output_path, exist_ok=True)
    
    def visualize(self, data, variable_name="Variable"):
        ...
        for label, (d, color) in data.items():
            ...
            label = f"{label} (mean={mean:.2f}, std={std:.2f}, skew={skewness:.2f}, kurtosis={kurtosis:.2f})"
            ax.hist(
                d, alpha=0.5, label=label, color=color, 
                bins=100, histtype='step', density=True, log=True
            )


        # Create info text box with statistics
        #info_text = "\n".join(
        #    [f"{label}: mean={s['mean']:.2f}, std={s['std']:.2f}, skewness={s['skewness']:.2f}, kurtosis={s['kurtosis']:.2f}"   
        #   = {s['std']:.2f}, skewness={s['skewness']:.2f}, kurtosis={s['kurtosis']:.2f}"   
        #   for label, s in stats.items()]
        # )
        #ax.text(0.05, 0.95, info_text, transform=ax.transAxes, fontsize=8, verticalalignment='top') 
        plt.legend(bbox_to_anchor=(0.5, -0.1), loc='center', fontsize=8)
        plt.grid(True, which="both", linestyle="-.", linewidth=0.5)
        plt.xlabel("Value")
        plt.ylabel("Density")
        plt.title(f"Histogram of {variable_name}")
        output_path = self.output_path + f"/histogram_{variable_name}.png"
        plt.savefig(output_path)
        plt.close()

    def compute(self, data: xr.DataArray):
        # Computes the histogram of a variable.
