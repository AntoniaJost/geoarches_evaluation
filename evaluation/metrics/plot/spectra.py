...
def plot_radial_spectrum(radial_spec, var, output_path, ref_spec=None):
    ...
    #wavelength.reverse()
    plt.loglog(wavelength, radial_spec, linewidth=2., c='black', label='Model projection')
    if ref_spec is not None:
        plt.loglog(wavelength, ref_spec, linewidth=2., linestyle='--', c='orange', label='Era5 Radial Spectrum')
    plt.title(f'{var}')
