...
from metrics.functional.utils import compute_anomaly, compute_mean, detrend_data
from metrics.functional.frequency_domain import compute_radial_spectrum, welch_psd
from metrics.functional import timeseries as eval_timeseries
...
from plot.spatial import (
    plot_variable,
    plot_temperature_with_geopotential_contours,
    plot_anomalies,
)
...
def plot_all_variables(
    fnc, data, variables, xticks, output_path, reference=None, std=None
):
    for var, lvl in variables:
        ...
        x_ref = (
            reference.sel(level=lvl)[var].to_numpy() if reference is not None else None
        )
        var_name = (
            surface_variables_short[var]
            if var in surface_variables_short.keys()
            else level_variables_short[var] + str(lvl)
        )
...
class ClimateEvaluator:
    ...
    def __init__(
        ...
        if isinstance(path, ListConfig):
            ...
            # list of members
            print(
                "Opening prediction data from multiple members:", path, " ...", end=" "
            )
            datasets = [
                xr.open_mfdataset(sorted(glob.glob(p + "/*.nc")), combine="by_coords")
                for p in path
            ]
        ...
        self.data = self.data.roll(
            longitude=-len(self.data.longitude) // 2, roll_coords=False
        )  # Roll longitude to match data
        ...
        if use_lat_weighting == "weatherbench":
            ...
            self.lat_weights = xr.DataArray(
                self.lat_weights, coords=[self.data.latitude], dims=["latitude"]
            )
        ...
        if reference_path is not None:
            ...
            if not reference_files:
                raise FileNotFoundError(
                    f"No reference files found for hour {hour} in {reference_path}"
                )
            ...
            self.reference = xr.open_mfdataset(
                reference_files, combine="by_coords", preprocess=preprocess
            )
            self.reference = self.reference.transpose(
                "time", "level", "latitude", "longitude"
            )
            self.reference = self.reference.reindex(
                latitude=list(reversed(self.reference.latitude))
            )
            # Select only variables that are in self.data

            # Roll data
            self.reference = self.reference.roll(
                longitude=-len(self.reference.longitude) // 2, roll_coords=False
            )  # Roll longitude to match data
    ...
    def select_reference_data(self, x, detrend=False):
        ...
        reference = reference.sel(
            time=(reference.time >= x.time[0]) & (reference.time <= x.time[-1])
        )
        ...
        if detrend:
            reference = detrend_data(
                reference,
                self.base_period,
                ["sea_surface_temperature", "2m_temperature"],
            )
    ...
    def bias_map(self, data, year=2020, season=None, **kwargs):
        ...
        assert self.reference is not None, (
            "Reference data is required for bias map computation."
        )
        ...
        if "sea_surface_temperature" in reference.data_vars:
            ...
            reference["sea_surface_temperature"] = (
                reference["sea_surface_temperature"].fillna(mean).ffill(dim="latitude")
            )
        ...
        if season is not None:
            ...
            reference = reference.sel(
                time=(reference.time >= data.time[0].astype("datetime64[D]"))
                & (reference.time <= data.time[-1].astype("datetime64[D]"))
            )
            print(data)
            print(reference)
            data = data.mean("time")
        ...
        for var, lvl in self.variables:
            ...
            var_name = self.get_variable_short_name(var, lvl)
            title = f"Bias Map {var_name} {year}" + (
                f" {season}" if season is not None else ""
            )
            fname = (
                f"bias_map_{var_name}_{year}"
                + (f"_{season}" if season is not None else "")
                + ".png"
            )
            plot_variable(
                x=x,
    ...
    def monthly_anomalies(self, data, detrend=False, year=2020):
        ...
        if self.reference is not None:
            mask = self.reference.isel(**{self.time_dim_name: 0}, drop=True)[
    ...
    def mean_climate(self, data, against_reference=False, **kwargs):
        ...
        if against_reference and self.reference is not None:
            reference = self.reference.sel(
                time=(self.reference.time >= data.time[0])
                & (self.reference.time <= data.time[-1])
            )
            reference = detrend_data(
                data=reference,
                base_period=self.base_period,
            ...
            warnings.warn(
                "Reference data not provided or not available. Skipping reference comparison."
            )
            reference = None
        else:
            reference = None
        ...
        output_path = os.path.join(
            self.output_path, "climeval", "monthly_mean", "plots"
        )
        ...
        for v, lvl in self.variables:
            plot_timeseries(
                ...
                ylabel=self.units[v],
                output_path=os.path.join(
                    output_path, f"monthly_mean_{level_variables_short[v]}_{lvl}.png"
                ),
    ...
    def _annual_cycle(self, data, detrend):
        ...
        annual_cycle = eval_timeseries.compute_annual_cycle(
            data, detrend, self.base_period
        )
        print("Done")
    ...
    def annual_cycle(self, data, detrend=False, against_reference=False, **kwargs):
        ...
        if against_reference and self.reference is not None:
            if "sea_surface_temperature" in data.data_vars:
                data["sea_surface_temperature"] = xr.where(
                    self.land_sea_mask == 0, data["sea_surface_temperature"], np.nan
                )

            if "sea_ice_cover" in data.data_vars:
                data["sea_ice_cover"] = xr.where(
                    self.land_sea_mask == 0, data["sea_ice_cover"], 0
                )
        ...
        if "std" in kwargs.keys():  # if ensemble
            ...
            std_annual_cycle = eval_timeseries.compute_annual_cycle(
                kwargs["std"], detrend
            )
        ...
        for v, lvl in self.variables:
            var_name = (
                surface_variables_short[v]
                if v in surface_variables_short.keys()
                else level_variables_short[v] + str(lvl)
            )
    ...
    def select_seasonal_data(self, year, season):
        ...
        if season not in seasons:
            raise ValueError(
                "Invalid season. Choose from 'DJF', 'MAM', 'JJA', or 'SON'."
            )
    ...
    def compute_mass_flux(self, data):
        ...
        # Constants
        const = 2.0 * self.R * self.pi * lat / self.g
    ...
    def compute_webster_yang_index(self, year=None, month=None):
        ...
        """
        data = (
            self.data.sel(time=slice(f"{year}-{month}-01", f"{year}-{month}-31"))
            if year and month
            else self.data
        )

        # Webster-Yang index
        u850 = data.u850.sel(latitude=slice(0, 20), longitude=slice(40, 110)).mean(
            dim=["latitude", "longitude"]
        )
        u200 = data.u200.sel(latitude=slice(0, 20), longitude=slice(40, 110)).mean(
            dim=["latitude", "longitude"]
        )
    ...
    def _compute_indian_monsoon_index(self, data, period, kinetic_energy=False):
        ...
        if kinetic_energy:
            v850_1 = data.sel(
                longitude=slice(220, 260), latitude=slice(15, 5), level=850
            )["v_component_of_wind"]
            v850_2 = data.sel(
                longitude=slice(250, 270), latitude=slice(30, 20), level=850
            )["v_component_of_wind"]
    ...
    def indian_monsoon_index(
        ...
        **kwargs,
        ...
        imd_index = self._compute_indian_monsoon_index(
            data, period, kinetic_energy=kinetic_energy
        )
        ...
        if against_reference and self.reference is not None:
            reference = self._compute_indian_monsoon_index(
                self.reference, period, kinetic_energy=kinetic_energy
            )
    ...
    def regression_analysis(self, data, index):
        ...
        regression_map = np.empty(
            (len(lat), len(lon))
        )  # Regression map to store slopes
    ...
    def _oni(self, data, period=None, power_spectrum=False, regression_analysis=False):
        ...
        nino34, sst = eval_timeseries.compute_oni_index(
            data=data, base_period=self.base_period, detrend=False
        )
    ...
    def covariance(self, variable1, variable2, era5_base=False, plot=True):
        ...
        if variable1_level is not None:
            if variable1_level not in self.data.level.values:
                raise ValueError(
                    f"Level {variable1_level} for variable {variable1_name} not found in the dataset. Available levels: {self.data.level.values}"
                )
            data = self.data.sel(level=variable1_level)
            era5 = (
                self.era5.sel(level=variable1_level)
                if era5_base and self.era5 is not None
                else None
            )
        ...
        if variable2_level is not None:
            if variable2_level not in self.data.level.values:
                raise ValueError(
                    f"Level {variable2_level} for variable {variable2_name} not found in the dataset. Available levels: {self.data.level.values}"
                )
            data = self.data.sel(level=variable2_level)
            era5 = (
                self.era5.sel(level=variable2_level)
                if era5_base and self.era5 is not None
                else None
            )
        ...
        print(
            f"Computing covariance between {variable1_name} and {variable2_name} at level {variable1_level} and {variable2_level} ...",
            end=" ",
        )
        ...
        if era5_base and era5 is not None:
            ...
            era5 = era5.sel(
                time=slice(
                    f"{self.base_period[0]}-01-01", f"{self.base_period[1]}-12-31"
                )
            )
            era5 = era5.fillna(
                value=era5.mean(dim=["latitude", "longitude"], skipna=True)
            )

            # compute covariance
            print(
                f"Computing covariance between {variable1_name} and {variable2_name} at level {variable1_level} and {variable2_level} for ERA5 ...",
                end=" ",
            )
    ...
    def correlation(
        ...
        print(
            f"Computing correlation between {variable1_name} and {variable2_name} at level {variable1_level} and {variable2_level} ...",
            end=" ",
        )
    ...
    def avg_temperature_map(self, level):
        ...
        for month in range(1, 13):
            temp = (
                self.data["2m_temperature"]
                .sel(time=self.data.time.dt.month == month)
                .mean(dim="time")
            )
            geopotential = (
                self.data["geopotential"]
                .sel(level=level, time=self.data.time.dt.month == month)
                .mean(dim="time")
            )
            ...
            plot_temperature_with_geopotential_contours(
                ...
                level,
                output_path,
                f"2m Temperature with Geopotential Heights at {level}hpa{month:02d}",
            )
    ...
    def latitude_time_plot(self, data, against_reference=False, ref_label="", **kwargs):
        ...
        if "time_period" in kwargs.keys():
            data = data.sel(
                time=slice(kwargs["time_period"][0], kwargs["time_period"][1])
            )
        print("Done")
        ...
        if ref_label == "":
            if (
                "sea_surface_temperature" in list(data.data_vars.keys())
                and self.land_sea_mask is not None
            ):
                print("Mask sea surface temperature over land ... ", end="")
                data["sea_surface_temperature"] = xr.where(
                    self.land_sea_mask > 0.5, np.nan, data["sea_surface_temperature"]
                )
        ...
        data["latitude"] = lat[
            ::-1
        ]  # to interpolate, lat has to be monotonically increasing
        ...
        data = compute_mean(
            data, reduce_dims=["time", "longitude"], groupby=["time.year", "time.month"]
        )
        ...
        for var, lvl in self.variables:
            ...
            plt.imshow(
                data.sel(level=lvl)[var],
                aspect="auto",
                interpolation="bilinear",
                cmap="coolwarm",
            )
            plt.yticks(
                ticks=np.arange(0, data.latitude.size, 10),
                labels=data.latitude.values[::10].round(2),
            )
            plt.xticks(
                ticks=np.arange(0, data.time.size, 12),
                labels=[
                    f"{y}"
                    for y in range(data["year"].values[0], data["year"].values[-1] + 1)
                ],
                rotation=90,
            )
            plt.colorbar(
                label=self.units[var],
            ...
            plt.ylabel("Latitude")
            plt.savefig(
                f"{output_path}/{ref_label}_latitude_time_{var_name}.png",
                bbox_inches="tight",
            )
            plt.close()
        ...
        if against_reference and self.reference is not None:
            ...
            self.latitude_time_plot(
                data=self.reference, against_reference=False, ref_label="era5", **kwargs
            )
    ...
    def annual_anomalies(self, data, against_reference=True, plot=True):
        ...
        anomaly = eval_timeseries.compute_annual_anomaly(data, True, self.base_period)
        ...
        if plot:
            ...
            for var, lvl in self.variables:
                ...
                plot_timeseries(
                    ...
                    output_path=os.path.join(
                        output_path, f"annual_anomalies_{var_name}.png"
                    ),
                )
        ...
        **kwargs,
        ...
        if against_reference:
            ...
            reference = reference.fillna(
                value=reference.mean(dim=["latitude", "longitude"], skipna=True)
            )
        ...
        data = (
            data.fillna(value=data.mean(dim=["longitude"], skipna=True))
            .ffill(dim="latitude")
            .bfill(dim="latitude")
        )
        ...
        for var, lvl in self.variables:
            ...
            plot_radial_spectrum(
                spec,
                var_name,
                output_path=output_path + f"/radial_spectrum_{var_name}_{ref_year}.png",
                ref_spec=ref_spec,
            )
            print("Done")
    ...
    def iter_variables(
        self, fnc, data, reference=None, stddev=None, variables=None, **kwargs
    ):
        """
