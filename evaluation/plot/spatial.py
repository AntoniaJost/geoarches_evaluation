...
def plot_anomalies(
    data, variables, levels, output_path, anomaly_type="Monthly", mask=None
):
    months = [
        "Jan",
        "Feb",
    ...
    for var in variables["surface"]:
        ...
        for i in range(anomaly.shape[0]):
            ...
            title = (
                f"{anomaly_type} Anomalies {months[i]} {surface_variables_short[var]} "
            )
            plot_variable(
                x,
                fname=f"{anomaly_type}_anomaly_{months[i]}_{surface_variables_short[var]}",
...
def plot_temperature_with_geopotential_contours(
    ...
    plt.savefig(
        f"{output_path}/2m_temp_geopotential_{level}.png", dpi=300, bbox_inches="tight"
    )
    plt.close(fig)
