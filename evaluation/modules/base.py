...
def load_batch_from_prediction(xrds: xr.Dataset, dataset: Era5Forecast):
    ...
    """
    if not all(
        [v[0] in xrds.dims.keys() for v in dataset.dimension_indexers.values()]
    ):  #
        # Respect dimension indexing of the loaded dataset (depends on dim names in dataset)
        name_map = {
            k: v[0]
            for k, v in dataset.dimension_indexers.items()
            if v[0] not in xrds.dims.keys()
    ...
    timestamp = torch.tensor(time // 10**9, dtype=torch.float64)
...
class AAIMIPRollout:
    def __init__(
        ...
        self.era5_ds = xr.open_mfdataset(
            files
        )  # to avoid multiprocessing issues with xarray
        ...
        if ablate_forcings:
            self.storage_path = os.path.join(
                storage_path, f"member_{member_id}", "ablate_forcings", "data"
            )
        else:
            self.storage_path = os.path.join(
                storage_path, f"member_{member_id}", "data"
            )
        ...
        if self.dataset.forcings_ds is not None:
            ...
            with xr.open_dataset(
                self.dataset.files[0], **self.dataset.xr_options
            ) as obs:
                self.land_sea_mask = obs["land_sea_mask"].to_numpy()[0]  # (1, lat, lon)
                self.land_sea_mask = torch.tensor(
                    self.land_sea_mask, dtype=torch.float32
                ).cuda()
            warn(
                "No forcings dataset found in the dataloader. Make sure this is intended."
            )
    ...
    def get_file_name(self, timestamp, member=None):
        ...
        """
        timestamp = (
            pd.to_datetime(timestamp.cpu(), unit="s").tz_localize(None).to_numpy()
        )
    ...
    def timestamp_to_batch(self, timestamp):
        ...
        idx -= (
            self.dataset.lead_time_hours // self.dataset.timedelta
        )  # account for prev_timestamp
        if idx < 0:
        ...
        if self.sst_scenario in ["2", "4"]:
            print(
                f"Applying SST scenario p{self.sst_scenario}K to state and prev state...",
                end="",
            )
            sst_index = self.dataset.variables["surface"].index(
    ...
    def get_initial_batch(self, start_timestamp):
        ...
        if self.continue_rollout and len(files) > 0:
            ...
            if self.module.embedder.forcings_embedding is not None:
                ...
                batch["future_forcings"] = (
                    self.monthly_forcings
                )  # placeholder, not used in current models

            print("Batch loaded from timestamp: ", timestamp)
            start_timestamp = timestamp

            print("Continuing rollout from timestamp: ", start_timestamp)
            self.store_initial_condition = (
                False  # Do not store initial condition if continuing rollout
            )
        else:
            if start_timestamp not in self.timestamps:
                raise ValueError(
                    f"Start timestamp {start_timestamp} not found in dataset timestamps."
                )
            print(
                "Preparing initial batch for timestamp: ", start_timestamp, end=" ... "
            )
    ...
    def get_forcings(self, timestamp):
        ...
        if self.dataset.forcings_mean is not None:
            forcings = (
                forcings.unsqueeze(1) - self.dataset.forcings_mean["forcings"]
            ) / self.dataset.forcings_std["forcings"]
            forcings = forcings.squeeze(1)  # remove batch dimension
    ...
    def compute_steps_per_years(self, start_timestamp, end_timestamp):
        ...
        if nyears <= 0:
            rollout_steps = (
                (end_timestamp - start_timestamp).astype("timedelta64[D]").astype(int)
            )
            if rollout_steps <= 0:
            ...
            if day != 31 and month != 12:
                ...
                last_day_of_first_year = (
                    start_timestamp.astype("datetime64[Y]")
                    + np.timedelta64(1, "Y")
                    - np.timedelta64(1, "D")
                ).astype("datetime64[D]")
                days_in_first_year = (
                    (last_day_of_first_year - start_timestamp.astype("datetime64[D]"))
    ...
    def dump_to_netcdf(self, output, timestamp):
        ...
        if self.storage_type == "monthly":
            ...
            xarrays["time"] = pd.to_datetime(
                timestamp.astype("datetime64[M]")
            ).tz_localize(None)
