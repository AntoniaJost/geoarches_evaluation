...
class GenerativeAAIMIPRollout(AAIMIPRollout):
    ...
    def _rollout(self, start_timestamp, rollout_steps, batch):
        ...
        loop_batch = TensorDict(
            {k: v.unsqueeze(0) for k, v in loop_batch.items()}, batch_size=[1]
        )
        ...
        for i, steps in enumerate(rollout_steps):
            ...
            outputs = [
                self.dataset.convert_to_xarray(
                    ...
                    loop_batch["timestamp"].detach().cpu()
                    + self.lead_time_hours * 3600 * (i + 1),
                    levels=arches_default_pressure_levels,
                )
            ...
            current_timestamp = start_timestamp + np.timedelta64(
                sum(rollout_steps[: i + 1]), "D"
            )
    ...
    def rollout(self, start_timestamp, end_timestamp, store_initial_condition):
        ...
        rollout_steps = (
            (end_timestamp - start_timestamp).astype("timedelta64[Y]").astype(int)
        )
