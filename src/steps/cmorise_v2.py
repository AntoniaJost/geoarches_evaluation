...
from logger import GREEN, RED

def _post_ncatted(path, var):
    """
    Run ncatted -O ... in-place on `path`, choosing the right flags
    ...
    try:
        ...
        print(f"⚠️ ncatted failed on {path}: {e}")
...
class CmoriseStep(Step):
    def run(self):
        ...
        frequency = {'monthly': 'mon', 'daily': 'day'} # for global attributes
        ...
        for scale in scales:
            ...
            # output_dir = os.path.join(out_base, scale, grid)
            ...
            for fname in files:
                ...
                # out_path = os.path.join(output_dir, var) # 4_cmorisation

                self.logger.info(f"[CmoriseStep] Processing {fname}")
                try:
                    ds = xr.open_dataset(fname)

                    # build aimip conform file name
